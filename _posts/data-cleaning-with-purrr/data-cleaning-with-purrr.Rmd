...
The `dir_ls` function from the `fs` package creates a vector of the paths for the files listed within the `rootpath`. Importantly, this vector is named with the associated paths, which will come in handy when using `map_df` below.
...
After create data frames for the demographics and FSQ data, they can then be joined by the `year` and `id`. 
