...
# If the user inputs "cat" for animal, the function will throw an error and say "Sorry, this function doesn't work for people who own cats"
...
This is obviously a silly example, but there are many reasons why you would want the function to output a warning or error. For example, you many want the function to warn someone if their input is not going to give them the output they expect, or if the function will not work for certain input. This will help the user work with your function to get their desired output.
...
Unfortunately, we get an error that says that the columns `var` and `grouping` are not found. This is because R is looking for these variables in your global environment but can't find them. Here are a couple solutions that use non-standard evaluation:
...
total_grouping <- function(data, var, grouping){
  ...
  a <- count(data, !!v1, !!v2) # Use !! and the the quoted variables
  b <- count(data, !!v1)
  b <- rename(b, "total" = "n")
  c <- left_join(a,b)
  na.omit(c)
