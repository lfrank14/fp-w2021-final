...
# R: I'm assuming that based on the posting order, this is the first post readers would encounter.
# It might be helpful to have an "About the Data" section here.

# R: I know this tutorial is specifically about writing functions, but I wonder if it
# could be helpful to include just a couple of brief sentences of introduction to what a
# function is and how it is used and maybe explain it in the context of a well-known base R
# function, such as `mean()`. That may be too basic for your target audience, though.
...
# R: I really like this example function! It feels very intuitive.
```
...
total_grouping <- function(data, var, grouping){
  v1 <- enquo(var) # I think some users may need an explanation of `enquo()`
  v2 <- enquo(grouping)
