...
# What is the whole data about? Maybe introduce the dataset a little bit?
```
...
The tutorial is clear and beginner friendly. The table of content on the side is helpful to navigate throughout the page. The examples provided here are straightforward. However, to make sure the tutorial even more beginner friendly, I would suggest add brief descriptions to some lines of code to explain the function used that line. For example, for some who is not familiar with `enquo()`, you can add a comment to explain what that line was doing. 